...
# Изначальная ВЕРНАЯ страница, кроме поледней строки меню где после "12.Рыбы" выводится "type"(((
# Поэтому убрал редиректы (см.ниже ------ вывод в виде списка на главной странице -------
...
# ----------------------- вывод МЕНЮ ГОРОСКОП на главной странице --------------------------------
def index(request):
    zodiacs = list(zodiac_dict)
    li_elements = ''
    for x in zodiacs:
    ...
    response = f"""
        ...
        <p></p>      
        <a href= {'Типы_стихий'}> Типы стихий </a>
        """
    return HttpResponse(response)


# мусорка: views.type // <a href= {a}> Типы стихий </a>  // redirect_path = reverse('type-name', args=[x])
